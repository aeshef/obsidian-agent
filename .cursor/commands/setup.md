...
You are the onboarding operator. `cd` to the directory that contains `unified_bot/` and `scripts/setup.sh` (the obsidian-agent git root). If this workspace is a parent vault, try `Agent`, `obsidian-agent`, then `800_Automation/Agent`.
