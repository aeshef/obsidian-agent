# Auto-generated for git-filter-repo
STUBS = {
...
def _is_prompt_path(filename):
    try:
        fn = filename.decode("utf-8")
    except Exception:
        fn = filename.decode("utf-8", "surrogateescape")
    if "/config/prompts/" not in fn and not fn.startswith("config/agent/prompts/"):
        return False
    return fn.endswith(".example.txt") or fn.endswith(".txt")

def _stub_bytes(fn):
    text = STUBS.get(fn) or (
        "# Prompt: "
        + fn.split("/")[-1].replace(".example.txt", "").replace(".txt", "")
        + "\n# Redacted from git history.\n"
    )
    return text.encode("utf-8")

def file_info_callback(filename, mode, blob_id, value):
    if not _is_prompt_path(filename):
        return (filename, mode, blob_id)
    try:
        fn = filename.decode("utf-8")
    except Exception:
        fn = filename.decode("utf-8", "surrogateescape")
    new_id = value.insert_file_with_contents(_stub_bytes(fn))
