...
(
    crontab -l 2>/dev/null | grep -vF "$MARKER" | grep -v 'kanban_monitor_standalone.py' \
        ...
        | grep -v 'cron_kanban_monitor.sh' | grep -v 'cron_routines.sh' \
        | grep -v 'cron_kanban_orphan_heal.sh' || true
    echo "$MARKER"
    ...
    echo "15 * * * * cd $BOT_ROOT && ./scripts/cron_kanban_orphan_heal.sh >> logs/kanban_orphan_heal.log 2>&1"
) > "$TMP"
crontab "$TMP"
rm -f "$TMP"
