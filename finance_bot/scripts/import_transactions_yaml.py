#!/usr/bin/env python3
"""
Import/restore transactions from YAML (append, dedupe by date+amount+description+account).
...
def main() -> None:
    ...
    default_account = str(data.get("account_name") or dmsg("finance_scripts", "import_default_account"))
    ...
    for raw in data.get("transactions") or []:
        ...
        category = str(raw.get("category") or dmsg("finance", "misc"))
    ...
    for raw in data.get("transfers") or []:
        ...
        description = str(
            raw.get("description")
            or dmsg("finance", "transfer_pair_description", from_account=from_name, to_account=to_name)
        )
        occurred = _occurred(y, m, day)
        from_id = _ensure_account(cur, args.user_id, from_name)
        to_id = _ensure_account(cur, args.user_id, to_name)
        exp_desc = dmsg("finance", "transfer_to_description", account=to_name)
        inc_desc = dmsg("finance", "transfer_from_description", account=from_name)
        if not _exists(cur, args.user_id, from_id, "expense", amount, occurred, exp_desc):
            cur.execute(
                """INSERT INTO transactions
                   (user_id, account_id, type, amount, currency, category, description, occurred_at, created_at)
                   VALUES (?, ?, 'expense', ?, 'RUB', ?, ?, ?, datetime('now'))""",
                (args.user_id, from_id, amount, transfer_cat, exp_desc, occurred),
            )
            added += 1
        ...
        if not _exists(cur, args.user_id, to_id, "income", amount, occurred, inc_desc):
            cur.execute(
                """INSERT INTO transactions
                   (user_id, account_id, type, amount, currency, category, description, occurred_at, created_at)
                   VALUES (?, ?, 'income', ?, 'RUB', ?, ?, ?, datetime('now'))""",
                (args.user_id, to_id, amount, transfer_cat, inc_desc, occurred),
            )
            added += 1
