...
Copy, trim, and post when `demo.gif` + green CI are ready.
...
**obsidian-agent** is a single Telegram process over planning / knowledge / finance. The Obsidian vault is canonical. Capabilities are fail-closed: if a module or connector is off, it disappears from UI, tools, prompts, and Mac↔VPS sync — not just a hidden menu item.

- Voice / photo / text capture with confirm-before-write for money
- Kanban as real markdown, not a bot todo list
- Optional health / calendar / broker connectors
