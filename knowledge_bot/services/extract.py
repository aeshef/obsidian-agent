...
from shared.asr import transcribe_audio as _shared_transcribe
...
def transcribe_av(path: Path, model_name: Optional[str] = None) -> str:
    """ASR для batch-пайплайна: HTTP → whisper(wav) → faster-whisper(singleton).

    Дефолтная модель "tiny" (экономия RAM на сервере; ASR_MODEL=small для качества).
    Возвращает "" при неудаче (пайплайн извлечения это допускает).
    """
    asr_lang_env = os.environ.get("ASR_LANGUAGE", "auto").strip()
    lang_pref = [p.strip() for p in asr_lang_env.split(",") if p.strip()] or None
    return _shared_transcribe(
        ...
        use_singleton=True,
        backend_order=("whisper", "faster_whisper"),
        convert_wav_for_whisper=True,
        lang_pref=lang_pref,
    )
